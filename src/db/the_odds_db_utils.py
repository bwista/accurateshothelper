...
NHL_API_BASE_URL = 'https://api.the-odds-api.com/v4/sports/icehockey_nhl'
NHL_API_HISTORICAL_URL = 'https://api.the-odds-api.com/v4/historical/sports/icehockey_nhl'
MLB_API_BASE_URL = 'https://api.the-odds-api.com/v4/sports/baseball_mlb'
MLB_API_HISTORICAL_URL = 'https://api.the-odds-api.com/v4/historical/sports/baseball_mlb'
...
def fetch_and_store_nhl_games(date=None, enable_logging=False):
    ...
    if use_historical:
        ...
        url = f"{NHL_API_HISTORICAL_URL}/events?{query_string}"
        if enable_logging:
        ...
        query_string = urllib.parse.urlencode(query_params)
        url = f"{NHL_API_BASE_URL}/events?{query_string}"
        if enable_logging:
    ...
    try:
        ...
        conn.commit()
        if enable_logging:
            logging.info(f"Inserted/Updated {len(records_to_insert)} records into nhl_game_info table.")
...
def fetch_and_store_mlb_games(date=None, enable_logging=False):
    """
    Fetch MLB events from the_odds API and store them in the PostgreSQL database.
    Uses historical API if date is provided, otherwise uses current events API.
    ...
    """
    if enable_logging:
        logging.info(f"Fetching and storing MLB events for date: {date}")
    ...
    # Construct API URL for MLB events
    if use_historical:
        query_params = {
            'apiKey': API_KEY,
            'date': end_time_utc.strftime('%Y-%m-%dT%H:%M:%SZ')
        ...
        query_string = urllib.parse.urlencode(query_params)
        url = f"{MLB_API_HISTORICAL_URL}/events?{query_string}"
        if enable_logging:
            logging.info(f"Using historical API for date: {date}")
    else:
        query_params = {
        ...
        query_string = urllib.parse.urlencode(query_params)
        url = f"{MLB_API_BASE_URL}/events?{query_string}"
        if enable_logging:
            logging.info("Using current events API")
    ...
    try:
        ...
        records_to_insert = [
            (
                event['id'],
                'baseball_mlb',  # Hardcoded since we're only dealing with MLB
                'MLB',           # Hardcoded sport title
                event['home_team'],
                event['away_team'],
                event['commence_time']
            )
        ...
        conn.commit()
        if enable_logging:
            logging.info(f"Inserted/Updated {len(records_to_insert)} records into mlb_game_info table.")
...
def get_mlb_events_from_db(query_date=None, enable_logging=False):
    """
    Retrieve MLB events from the PostgreSQL the_odds database for a given date.
    ...
    """
    if enable_logging:
        logging.info(f"Retrieving MLB events from DB for date: {query_date}")
    if query_date is None:
        query_date = datetime.now()
    else:
        query_date = datetime.strptime(query_date, '%Y-%m-%d')
...
def get_player_sog_odds(player_name=None, query_date=None, sportsbook=None, team_name=None, line=False, fuzzy_threshold=85):
    ...
    if not games:
        print(f"No games found in the nhl_game_info table for date {query_date}.")
        return []
    ...
    try:
        ...
        if not odds_dict:
            ...
            print(f"No odds found in nhl_player_sog_odds table for game_ids {game_ids} with filters: {filter_str}")
            return []
...
def process_sog_markets(game_id, query_date=None, enable_logging=False):
    ...
    if use_historical:
        ...
        base_url = f"{NHL_API_HISTORICAL_URL}/events/{game_id}/odds"
        if enable_logging:
            logging.info(f"Using historical odds for date: {query_params['date']}")
    else:
        base_url = f"{NHL_API_BASE_URL}/events/{game_id}/odds"
        if enable_logging:
    ...
    try:
        # Define the insert query
        insert_query = """
            INSERT INTO nhl_player_sog_odds (
                game_id,
                sportsbook,
                player_name,
                market_type,
...
def process_saves_markets(game_id, query_date=None, enable_logging=False):
    ...
    try:
        # Define the insert query
        insert_query = """
            INSERT INTO nhl_player_saves_odds (
                game_id,
                sportsbook,
                player_name,
                market_type,
...
def process_moneyline_markets(game_id, query_date=None, enable_logging=False):
    ...
    try:
        # Define the insert query
        insert_query = """
            INSERT INTO nhl_moneyline_odds (
                game_id,
                sportsbook,
                team_name,
                price,
...
def get_mismatched_game_ids_with_details(table_name='nhl_player_sog_odds', enable_logging=False):
    """
    Compare distinct game_ids in nhl_game_info and a specified odds table
    and return game_ids that are not present in both tables, along with
    additional details for missing games in nhl_game_info.

    Args:
        table_name (str): The name of the odds table to compare with nhl_game_info.
                         Must be either 'nhl_player_sog_odds' or 'nhl_moneyline_odds'.
        enable_logging (bool): If True, enables logging. Defaults to False.
    ...
    """
    if enable_logging:
        logging.info(f"Comparing distinct game_ids in nhl_game_info and {table_name} tables.")

    # Validate table name
    valid_tables = {'nhl_player_sog_odds', 'nhl_moneyline_odds'}
    ...
    try:
        ...
        cursor.execute("SELECT DISTINCT id FROM nhl_game_info")
        ...
        # Fetch additional details for game_ids only in nhl_game_info
        only_in_game_info = []
        ...
        if enable_logging:
            logging.info(f"Game IDs only in nhl_game_info: {only_in_game_info}")
    ...
    try:
        ...
        if not odds_list:
            ...
            if sportsbook:
                filters.append(f"sportsbook '{sportsbook}'")
            filter_str = " and ".join(filters)
            print(f"No odds found in nhl_moneyline_odds table for game_ids {game_ids} with filters: {filter_str}")
            return []
...
def get_pitcher_strikeout_odds(pitcher_name=None, query_date=None, sportsbook=None, team_name=None, line=False, fuzzy_threshold=85):
    """
    Retrieve pitcher strikeout odds from the PostgreSQL the_odds database with fuzzy matching for pitcher names.

    Args:
        pitcher_name (str, optional): The full name of the pitcher to filter odds by.
        query_date (str, optional): The date to query in 'YYYY-MM-DD' format. Used to retrieve games.
        sportsbook (str, optional): The name of the sportsbook to filter odds by.
        team_name (str, optional): The full name of the team to filter games by.
        line (bool, optional): If True, filters odds to find those closest to +100. Defaults to False.
    ...
    Returns:
        list: A list of dictionaries containing pitcher strikeout odds.
    """
    logging.info(f"Retrieving pitcher strikeout odds for pitcher: {pitcher_name}, date: {query_date}, sportsbook: {sportsbook}, team: {team_name}, line: {line}")

    # Validate that at least one filter is provided
    if not pitcher_name and not sportsbook and not team_name:
        print("At least one of pitcher_name, sportsbook, or team_name must be provided.")
        return []

    # Convert query_date to UTC if provided
    if query_date:
        # Convert date string to UTC ISO8601 format
        query_date_utc = convert_to_utc_iso8601(query_date, default_timezone=ZoneInfo("America/Chicago"), enable_logging=True)
        if not query_date_utc:
            print(f"Invalid date format: {query_date}")
            return []
        query_date = query_date_utc

    # Retrieve games from the database for the given date
    games = get_mlb_events_from_db(query_date, enable_logging=True)

    if not games:
        print(f"No games found in the mlb_game_info table for date {query_date}.")
        return []
    ...
    try:
        ...
        # If pitcher_name is provided, first get all distinct pitcher names from the database
        # for the specified game_ids to perform fuzzy matching
        if pitcher_name:
            cursor.execute("""
                SELECT DISTINCT player_name 
                FROM mlb_pitcher_strikeouts 
                WHERE game_id = ANY(%s)
            """, (game_ids,))
            db_pitchers = [row[0] for row in cursor.fetchall()]
            
            # Perform fuzzy matching to find the best match
            best_match = None
            best_score = 0
            for db_pitcher in db_pitchers:
                score = fuzz.ratio(pitcher_name.lower(), db_pitcher.lower())
                if score > best_score and score >= fuzzy_threshold:
                    best_score = score
                    best_match = db_pitcher
            
            if best_match:
                logging.info(f"Fuzzy matched '{pitcher_name}' to '{best_match}' with score {best_score}")
                pitcher_name = best_match
            else:
                logging.warning(f"No fuzzy match found for pitcher '{pitcher_name}' above threshold {fuzzy_threshold}")
                return []
        ...
        base_query = """
            SELECT 
                ...
                pso.price,
                pso.last_update
            FROM mlb_pitcher_strikeouts pso
            WHERE pso.game_id = ANY(%s)
        """
        ...
        if not odds_dict:
            filters = []
            if pitcher_name:
                filters.append(f"pitcher '{pitcher_name}'")
            if sportsbook:
                filters.append(f"sportsbook '{sportsbook}'")
            if team_name:
                filters.append(f"team '{team_name}'")
            filter_str = " and ".join(filters)
            print(f"No odds found in mlb_pitcher_strikeouts table for game_ids {game_ids} with filters: {filter_str}")
            return []

        # Filter for odds closest to +100 if the line flag is set
        result = filter_odds_closest_to_100(odds_dict) if line else [odds for odds_list in odds_dict.values() for odds in odds_list]
        logging.info("Completed retrieving pitcher strikeout odds.")
        return result
...
def process_strikeout_markets(game_id, query_date=None, enable_logging=False):
    """
    Fetches and processes pitcher strikeout markets from the_odds API for a specific game.
    Uses historical odds if query_date is not today.
    ...
    """
    if enable_logging:
        logging.info(f"Processing strikeout markets for game: {game_id}")
    ...
    if query_date and isinstance(query_date, str):
        # If query_date is not in ISO8601 format (no 'T' and 'Z'), fetch game info
        if 'T' not in query_date and 'Z' not in query_date:
            games = get_mlb_events_from_db(query_date, enable_logging=enable_logging)
            game_info = next((game for game in games if game['id'] == game_id), None)
            if game_info:
                # Convert commence_time to UTC ISO8601 string
                query_date = game_info['commence_time'].astimezone(timezone.utc).strftime('%Y-%m-%dT%H:%M:%SZ')
    ...
    # Construct API URL for pitcher strikeout markets
    query_params = {
        'apiKey': API_KEY,
        'regions': 'us',
    ...
    if use_historical:
        ...
        base_url = f"{MLB_API_HISTORICAL_URL}/events/{game_id}/odds"
        if enable_logging:
            logging.info(f"Using historical odds for date: {query_params['date']}")
    else:
        base_url = f"{MLB_API_BASE_URL}/events/{game_id}/odds"
        if enable_logging:
            logging.info("Using live odds")
    ...
    try:
        # Define the insert query
        insert_query = """
            INSERT INTO mlb_pitcher_strikeouts (
                game_id,
                sportsbook,
                player_name,
                market_type,
    ...
    if enable_logging:
        logging.info(f"Completed processing strikeout markets for game {game_id}")

def process_all_strikeout_markets(query_date=None, enable_logging=False):
    """
    Process strikeout markets for all games on a given date.
    ...
    """
    if enable_logging:
        logging.info(f"Processing all strikeout markets for date: {query_date}")
    ...
    if not games:
        if enable_logging:
            logging.info(f"No games found for date {query_date}, attempting to fetch from API")
        fetch_and_store_mlb_games(query_date, enable_logging=enable_logging)
        # Try to get games again after fetching
        games = get_mlb_events_from_db(query_date, enable_logging=enable_logging)
    ...
    for game in games:
        ...
        if enable_logging:
            logging.info(f"Processing game {game['id']} scheduled for {utc_time}")
        process_strikeout_markets(game['id'], query_date=utc_time, enable_logging=enable_logging)

    if enable_logging:
        logging.info(f"Completed processing all strikeout markets for date {query_date}")

def process_all_strikeout_markets_date_range(start_date, end_date, enable_logging=False):
    """
    Process strikeout markets for all games within a date range (inclusive).
    ...
    """
    if enable_logging:
        logging.info(f"Processing all strikeout markets from {start_date} to {end_date}")
    ...
    try:
        ...
        while current <= end:
            ...
            # Process all strikeout markets for the current date
            process_all_strikeout_markets(current_date, enable_logging=enable_logging)
            
            # Move to next day
            current += timedelta(days=1)
            
        if enable_logging:
            logging.info(f"Completed processing all strikeout markets from {start_date} to {end_date}")
